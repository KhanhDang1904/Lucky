...
class exportExcelDanhSachPhieuLuong
{
    ...
    public function renderBody($activeSheet){
        ...
        foreach ($data['data'] as $item) {
            $activeSheet
                ->setCellValue("A{$dong}",$item['id'])
                ->setCellValue("B{$dong}", $item['hoten'])
                ->setCellValue("C{$dong}",$item['dien_thoai'])
                ->setCellValue("D{$dong}",$item['tong_tien']);
            $dong++;
        }
        $dong--;
        formatExcel::setBorder($activeSheet, "A5:D{$dong}");
        $activeSheet
    ...
    public function run()
    {
        $objPHPExcel = \PhpOffice\PhpSpreadsheet\IOFactory::load(dirname(dirname(__DIR__)).'/common/template/BaoCaoDanhSachPhieuLuong.xlsx');
